...
def get_commands(args: argparse.Namespace) -> CommandInfo | int:
    mw_versions = os.popen('/usr/bin/getMWVersions all').read().strip()
    versions = {}
    if mw_versions:
    ...
    if not args.version:
        ...
        if not dbname:
            dbname = 'default'
        args.version = os.popen(f'/usr/bin/getMWVersion {dbname}').read().strip()
    ...
    if wiki == 'all':
        long = True
        command = f'sudo -u {args.runas} /usr/bin/foreachwikiindblist /srv/mediawiki/cache/databases.php {script}'
    elif wiki and wiki in validDBLists:
        long = True
        command = f'sudo -u {args.runas} /usr/bin/foreachwikiindblist /srv/mediawiki/cache/{wiki}.php {script}'
    elif args.extension:
        long = True
        ...
        command = f'sudo -u {args.runas} /usr/bin/foreachwikiindblist /tmp/{args.extension}.php {script}'
    else:
...
def run(info: CommandInfo) -> None:  # pragma: no cover
    logcommand = f'/usr/bin/logsalmsg "{info["command"]}'
    print('Will execute:')
    if info['generate']:
        print(info['generate'])
    print(info['command'])
